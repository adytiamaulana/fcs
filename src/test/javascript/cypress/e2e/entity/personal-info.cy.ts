...
describe('PersonalInfo e2e test', () => {
  ...
  describe('new PersonalInfo page', () => {
    ...
    it('should create an instance of PersonalInfo', () => {
      cy.get(`[data-cy="name"]`).type('speedily nod');
      cy.get(`[data-cy="name"]`).should('have.value', 'speedily nod');

      cy.get(`[data-cy="gender"]`).type('yet');
      cy.get(`[data-cy="gender"]`).should('have.value', 'yet');

      cy.get(`[data-cy="birthDate"]`).type('resolution besides near');
      cy.get(`[data-cy="birthDate"]`).should('have.value', 'resolution besides near');

      cy.get(`[data-cy="telephone"]`).type('(812) 322-3388 x7683');
      cy.get(`[data-cy="telephone"]`).should('have.value', '(812) 322-3388 x7683');

      cy.get(`[data-cy="createdBy"]`).type('mute where silence');
      cy.get(`[data-cy="createdBy"]`).should('have.value', 'mute where silence');

      cy.get(`[data-cy="createdAt"]`).type('meh at');
      cy.get(`[data-cy="createdAt"]`).should('have.value', 'meh at');

      cy.get(`[data-cy="updatedBy"]`).type('soon aha but');
      cy.get(`[data-cy="updatedBy"]`).should('have.value', 'soon aha but');

      cy.get(`[data-cy="updatedAt"]`).type('woot times');
      cy.get(`[data-cy="updatedAt"]`).should('have.value', 'woot times');

      cy.get(`[data-cy="deletedBy"]`).type('excluding than');
      cy.get(`[data-cy="deletedBy"]`).should('have.value', 'excluding than');

      cy.get(`[data-cy="deletedAt"]`).type('midst');
      cy.get(`[data-cy="deletedAt"]`).should('have.value', 'midst');
