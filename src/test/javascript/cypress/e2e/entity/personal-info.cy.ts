...
describe('PersonalInfo e2e test', () => {
  ...
  const personalInfoSample = { createdBy: 'vainly status', createdAt: '2024-08-12T05:33:49.611Z' };
  ...
  describe('new PersonalInfo page', () => {
    ...
    it('should create an instance of PersonalInfo', () => {
      cy.get(`[data-cy="name"]`).type('yuck');
      cy.get(`[data-cy="name"]`).should('have.value', 'yuck');

      cy.get(`[data-cy="gender"]`).type('ugh dapper');
      cy.get(`[data-cy="gender"]`).should('have.value', 'ugh dapper');

      cy.get(`[data-cy="birthDate"]`).type('call er');
      cy.get(`[data-cy="birthDate"]`).should('have.value', 'call er');

      cy.get(`[data-cy="telephone"]`).type('902-777-0341 x595');
      cy.get(`[data-cy="telephone"]`).should('have.value', '902-777-0341 x595');

      cy.get(`[data-cy="branch"]`).type('20763');
      cy.get(`[data-cy="branch"]`).should('have.value', '20763');

      cy.get(`[data-cy="createdBy"]`).type('liar painfully because');
      cy.get(`[data-cy="createdBy"]`).should('have.value', 'liar painfully because');

      cy.get(`[data-cy="createdAt"]`).type('2024-08-12T08:49');
      cy.get(`[data-cy="createdAt"]`).blur();
      cy.get(`[data-cy="createdAt"]`).should('have.value', '2024-08-12T08:49');

      cy.get(`[data-cy="updatedBy"]`).type('conscious');
      cy.get(`[data-cy="updatedBy"]`).should('have.value', 'conscious');

      cy.get(`[data-cy="updatedAt"]`).type('2024-08-11T20:34');
      cy.get(`[data-cy="updatedAt"]`).blur();
      cy.get(`[data-cy="updatedAt"]`).should('have.value', '2024-08-11T20:34');

      cy.get(`[data-cy="deletedBy"]`).type('briskly');
      cy.get(`[data-cy="deletedBy"]`).should('have.value', 'briskly');

      cy.get(`[data-cy="deletedAt"]`).type('2024-08-12T00:40');
      cy.get(`[data-cy="deletedAt"]`).blur();
      cy.get(`[data-cy="deletedAt"]`).should('have.value', '2024-08-12T00:40');
