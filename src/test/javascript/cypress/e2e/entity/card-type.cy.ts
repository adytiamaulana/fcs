...
describe('CardType e2e test', () => {
  ...
  describe('new CardType page', () => {
    ...
    it('should create an instance of CardType', () => {
      cy.get(`[data-cy="cardCode"]`).type('8589');
      cy.get(`[data-cy="cardCode"]`).should('have.value', '8589');

      cy.get(`[data-cy="cardName"]`).type('underlie for');
      cy.get(`[data-cy="cardName"]`).should('have.value', 'underlie for');

      cy.get(`[data-cy="createdBy"]`).type('ouch majestically');
      cy.get(`[data-cy="createdBy"]`).should('have.value', 'ouch majestically');

      cy.get(`[data-cy="createdAt"]`).type('frayed');
      cy.get(`[data-cy="createdAt"]`).should('have.value', 'frayed');

      cy.get(`[data-cy="updatedBy"]`).type('mmm flood');
      cy.get(`[data-cy="updatedBy"]`).should('have.value', 'mmm flood');

      cy.get(`[data-cy="updatedAt"]`).type('as international ah');
      cy.get(`[data-cy="updatedAt"]`).should('have.value', 'as international ah');

      cy.get(`[data-cy="deletedBy"]`).type('nuke round model');
      cy.get(`[data-cy="deletedBy"]`).should('have.value', 'nuke round model');

      cy.get(`[data-cy="deletedAt"]`).type('but');
      cy.get(`[data-cy="deletedAt"]`).should('have.value', 'but');
