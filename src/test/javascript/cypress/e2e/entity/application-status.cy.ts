...
describe('ApplicationStatus e2e test', () => {
  ...
  const applicationStatusSample = { code: 'beetle bump', createdBy: 'however gadzooks well-being', createdAt: '2024-08-12T09:04:44.212Z' };
  ...
  describe('new ApplicationStatus page', () => {
    ...
    it('should create an instance of ApplicationStatus', () => {
      cy.get(`[data-cy="code"]`).type('wetly preserve assurance');
      cy.get(`[data-cy="code"]`).should('have.value', 'wetly preserve assurance');

      cy.get(`[data-cy="status"]`).type('desecrate flashy');
      cy.get(`[data-cy="status"]`).should('have.value', 'desecrate flashy');

      cy.get(`[data-cy="createdBy"]`).type('autumn kazoo ick');
      cy.get(`[data-cy="createdBy"]`).should('have.value', 'autumn kazoo ick');

      cy.get(`[data-cy="createdAt"]`).type('2024-08-12T05:31');
      cy.get(`[data-cy="createdAt"]`).blur();
      cy.get(`[data-cy="createdAt"]`).should('have.value', '2024-08-12T05:31');

      cy.get(`[data-cy="updatedBy"]`).type('diligently for if');
      cy.get(`[data-cy="updatedBy"]`).should('have.value', 'diligently for if');

      cy.get(`[data-cy="updatedAt"]`).type('2024-08-11T22:13');
      cy.get(`[data-cy="updatedAt"]`).blur();
      cy.get(`[data-cy="updatedAt"]`).should('have.value', '2024-08-11T22:13');

      cy.get(`[data-cy="deletedBy"]`).type('crowded anenst');
      cy.get(`[data-cy="deletedBy"]`).should('have.value', 'crowded anenst');

      cy.get(`[data-cy="deletedAt"]`).type('2024-08-11T18:32');
      cy.get(`[data-cy="deletedAt"]`).blur();
      cy.get(`[data-cy="deletedAt"]`).should('have.value', '2024-08-11T18:32');
