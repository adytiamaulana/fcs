...
describe('ApplicationStatus e2e test', () => {
  ...
  describe('new ApplicationStatus page', () => {
    ...
    it('should create an instance of ApplicationStatus', () => {
      cy.get(`[data-cy="code"]`).type('infection bitterly via');
      cy.get(`[data-cy="code"]`).should('have.value', 'infection bitterly via');

      cy.get(`[data-cy="status"]`).type('at digitalize');
      cy.get(`[data-cy="status"]`).should('have.value', 'at digitalize');

      cy.get(`[data-cy="createdBy"]`).type('comment');
      cy.get(`[data-cy="createdBy"]`).should('have.value', 'comment');

      cy.get(`[data-cy="createdAt"]`).type('slippery detest');
      cy.get(`[data-cy="createdAt"]`).should('have.value', 'slippery detest');

      cy.get(`[data-cy="updatedBy"]`).type('unfortunately ack');
      cy.get(`[data-cy="updatedBy"]`).should('have.value', 'unfortunately ack');

      cy.get(`[data-cy="updatedAt"]`).type('mileage incidentally where');
      cy.get(`[data-cy="updatedAt"]`).should('have.value', 'mileage incidentally where');

      cy.get(`[data-cy="deletedBy"]`).type('armoire ouch which');
      cy.get(`[data-cy="deletedBy"]`).should('have.value', 'armoire ouch which');

      cy.get(`[data-cy="deletedAt"]`).type('census');
      cy.get(`[data-cy="deletedAt"]`).should('have.value', 'census');
