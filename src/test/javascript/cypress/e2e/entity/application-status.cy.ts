...
describe('ApplicationStatus e2e test', () => {
  ...
  const applicationStatusSample = { code: 'destock ah', createdBy: 'sonnet oof', createdAt: '2024-08-12T10:49:45.112Z' };
  ...
  describe('new ApplicationStatus page', () => {
    ...
    it('should create an instance of ApplicationStatus', () => {
      cy.get(`[data-cy="code"]`).type('demanding for');
      cy.get(`[data-cy="code"]`).should('have.value', 'demanding for');

      cy.get(`[data-cy="status"]`).type('only');
      cy.get(`[data-cy="status"]`).should('have.value', 'only');

      cy.get(`[data-cy="createdBy"]`).type('instead silky helplessly');
      cy.get(`[data-cy="createdBy"]`).should('have.value', 'instead silky helplessly');

      cy.get(`[data-cy="createdAt"]`).type('2024-08-11T22:42');
      cy.get(`[data-cy="createdAt"]`).blur();
      cy.get(`[data-cy="createdAt"]`).should('have.value', '2024-08-11T22:42');

      cy.get(`[data-cy="updatedBy"]`).type('fuzzy woefully interfere');
      cy.get(`[data-cy="updatedBy"]`).should('have.value', 'fuzzy woefully interfere');

      cy.get(`[data-cy="updatedAt"]`).type('2024-08-12T06:55');
      cy.get(`[data-cy="updatedAt"]`).blur();
      cy.get(`[data-cy="updatedAt"]`).should('have.value', '2024-08-12T06:55');

      cy.get(`[data-cy="deletedBy"]`).type('excepting');
      cy.get(`[data-cy="deletedBy"]`).should('have.value', 'excepting');

      cy.get(`[data-cy="deletedAt"]`).type('2024-08-12T09:17');
      cy.get(`[data-cy="deletedAt"]`).blur();
      cy.get(`[data-cy="deletedAt"]`).should('have.value', '2024-08-12T09:17');
